...
function f2() {
    ...
    for (let i = 0; i < a2.length; i++){
        out.innerHTML += `Индекс ${i}; Значение ${a2[i]} <br>`; 
    }
...
function f6() {
    let a6_1 = [];
    for (let keyOut in a6){
        for (let keyIn in a6[keyOut]){
            
            
            a6_1.push(a6[keyOut][keyIn]);
        }
    }
    a6 = a6_1;
    for (let i = 0; i < a6.length; i++){
        document.querySelector('.out-6').textContent += a6[i] + ' ';
    }
...
function f7() {
let a7_1 = new Object();
   for (let object of a7){
       a7_1[object.id] = object.name;
   }
   return a7_1;
}
...
function f9() {
    ...
    for (let i = 0; i < a9.length; i++){
        if(a9[i].length > maxIndex){
            maxIndex = a9[i].length;
        }
    }
    return maxIndex - 1;
}
...
function f11() {
    for (let item in a11){
        if (a11[item] > 10){
            document.querySelector('.out-11').textContent += `${a11[item]} `;
        }
    }
}
...
function f13() {
    for (let word of a13){
        document.querySelector('.out-13').textContent += word + ' ';
    }
}
